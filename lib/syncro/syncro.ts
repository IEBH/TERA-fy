...
export default class Syncro {
	...
	*/
	setHeartbeat(enable: boolean = true, options?: any): Promise<void> | void { // Return type adjusted
		let settings = {
			immediate: false, // Default immediate to false, override in mount
			...options,
		};
	...
	* @returns {Promise} A promise which resolves when the operation has completed
	*/
	flush(options?: any): Promise<void> {
		let settings = {
			destroy: false,
			...options,
		};
		...
		return fetch(`${this.config.syncroRegistryUrl}/${this.path}/flush` + (settings.destroy ? '?destroy=1' : ''))
			.then(response => {
				if (!response.ok) throw new Error(response.statusText || 'An error occured');
			});
	}
