...
export default class TERAFile {
	...
	*/
	constructor(base) {
		...
		// Check raw data
		['path', 'created', 'modified', 'accessed', 'size'].forEach(key => {
			if (!this._data[key]) throw new Error(`new TERAFile(...) requires key "${key}"`);
		});
	...
	*/
	getContents() {
		return Promise.resolve()
			...
			.then(res => res.ok
				? res.blob()
				: Promise.reject(`Unable to fetch ${this.path} - ${e.toString()}`, {cause: e})
			)
	}
