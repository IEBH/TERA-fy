...
export default class SyncroKeyed extends Syncro {
	...
	*/
	mount() {
		...
		return Promise.resolve()
			// Mount all members by looking for similar keys {{{
	...
	*/
	async keyedAssign(state) {
		...
		let isBlank = this.members.length == 1 && Object.keys(this.members[0]).length == 0;

		if (isBlank) {
			let chunks = chunk(Object.keys(state), this.keyedCOnfig.maxKeys)
				.map(chunk => chunk.map(k => state[k]))
			...
			await Promise.all(
				chunks.map(async (chunk, chunkIndex) => {
					...
					if (!(chunkIndex in this.members[chunkIndex])) await this.keyedMembersExtend(chunkIndex);

					// Populate its state
					Object.assign(this.members[chunkIndex].value, chunk)
				})
			);

		} else { // Non-blank - call keyedSet() on each key of state, merging slowly
			Object.entries(state)
				.forEach(([key, val]) => this.keyedSet(key, val))
		}
	}
	...
	*/
	async keyedMembersExpand(index) {
		...
		let memberId = fsId.replace('*', ''+index);
		this.debug('Expand SyncroKeyed to', fsCollection, '#', fsId, '@ Index=', index);

		// Create a new Syncro member, inheriteing some details from this parent item
		let syncro = new Syncro(`${fsCollection}::${memberId}`, {
			debug: this.debug,
