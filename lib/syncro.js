...
export default class Syncro {
	...
	* @param {Object} [options.context] Additional named parameters to pass to callbacks like initState
	...
	mount(options) {
		...
		return Promise.resolve()
			...
			.then(()=> { // Optionally create the doc if it has no content
				...
				if (settings.initialState) { // Use provided state instead of trying to populate
					return this.setFirestoreState(settings.initialState, {method: 'set'});
				}

				this.debug('Populate initial Syncro state');
				...
				return Promise.resolve()
					...
					.then(state => this.setFirestoreState(state)) // Send new base state to Firestore
			})
...
export const syncEntities = {
	projects: {
		...
		async initState({supabase, id, tera}) {
			if (!tera) throw new Error('initState() for projects requires `{tera:Object}`');
