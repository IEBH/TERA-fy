...
export default class TeraFyServer {
	...
	*
	* @returns {Promise<*>} A promise which resolves to the eventual input value
	*/
	setProjectStateDefaults(path, value, options) {
		if (!app.service('$projects').active) throw new Error('No active project');

		debugger; // This should use the default _pathSet() on the server always, not use the (possibly defunct) subclass of _pathSet
	...
	setProjectLibrary(path, refs, options) {
		...
		return Promise.resolve()
			...
			.then(()=> { // Correct filename to a full path if needed
				...
				this.debug('Save citation library as', path);
			})
			.then(()=> {
