	...
	*/
	requireUser() {
		return this.getUser()
			...
			.then(() => { // Only embedded mode - try to login the user via Kinde
				debugger;
				if (this.settings.serverMode != TeraFyServer.SERVERMODE_EMBEDDED) return;
			...
			.then(()=> this.requireUser()) // User escaped this promise chain somehow - loop around
			.catch(e => {
				if (e === 'EXIT') return; // Exit with a valid user
				throw e;
			})
	}
