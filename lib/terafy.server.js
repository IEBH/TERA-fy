...
export default class TeraFyServer {
	...
	*/
	setProjectLibrary(path, refs, options) {
		debugger; // FIXME: Untested without path
		...
		return Promise.resolve()
			...
			.then(()=> {
				...
				debugger; // FIXME: Untested when prompting
				...
				return this.selectProjectFile({
					...
					.then(path => {
						this.debug('Will save incomming refs as', path);
						settings.path = path;
					})
			})
			.then(()=> {
				...
				switch (settings.format) {
					...
					case 'blob':
						...
						return new File([settings.refs], app.service('$supabase')._parsePath(path).basename);
					case 'file':
